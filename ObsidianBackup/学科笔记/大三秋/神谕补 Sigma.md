...
>2014版请将每个页数-4或-5
>Ver Sigma：补充了食用方法和一部分概念的解释，参考了历年卷
...
**1.4 操作系统的主要功能（17）**：
- 操作系统最基本的功能就是：
	1. **处理机管理功能（18）：包括进程控制、进程同步、进程通信和调度；利用储存器构造进程和线程**
	2. **储存器管理功能（19）：包括内存分配、内存保护、地址映射和内存扩充；利用储存器构造进程虚拟地址空间**
	3. **设备管理功能（20）：包括缓冲管理，设备分配和程序处理；统一设备操作界面**
	4. **文件管理功能（21）：包括文件储存空间的管理、目录管理和文件的读/写管理和保护；利用储存设备构造文件抽象**
...
**2.8.1 线程实现的方式（85）**：
- 1.内核支持线程KST：效率高，但用户的线程切换开销大，**在内核中实现线程**
- 2.用户级线程ULT：节约了转换的开销、独特的调度算法、广泛支持，但线程阻塞会直接导致进程阻塞，只能分到一个CPU，**在库函数中实现线程**
- 3.组合方式：将用户线程映射至核心线程
...
- 9.4.1 系统调用的基本概念：
	- 系统运行的状态：在处理机的运行过程分成**系统态和用户态（316）**：
		...
		- 为什么要分成特权和非特权指令两种状态：实施操作系统保护，使得只有操作系统才能够执行特权指令，而应用程序不能执行特权指令
